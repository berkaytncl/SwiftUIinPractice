...
//                let products = self.products.filter({ $0.brand == brand })
                rows.append(ProductRow(title: brand?.capitalized ?? Product.mock.brand!, products: products.shuffled()))
            }
            productRows = rows
        } catch {
            print(error)
    ...
    private func heroCell(product: Product) -> some View {
        NetflixHeroCell(
            ...
            isNetflixFilm: true,
            title: product.title,
            categories: [product.category.capitalized, product.brand ?? Product.mock.brand!],
            onBackgroundPressed: {
